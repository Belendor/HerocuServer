...
router.route("/update/:id").post((req,res) =>{
    Exercise.findById(req.params.id).then(
        exercise => {
            ...
            exercise.describtion = req.body.describtion;
            exercise.duration = Number(req.body.duration)
            exercise.date = Date.parse(req.body.date)

            exercise.save().then(()=> res.json("exercise saved!")).catch(err => res.json("Error: " + err))

        }).catch(err => res.json("Error in find ID: " + err))
})
